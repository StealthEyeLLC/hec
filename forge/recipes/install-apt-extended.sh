...
if [[ ${EUID:-$(id -u)} -ne 0 ]]; then
  echo "install-apt-extended.sh must run as root" >&2
  exit 1
fi
...
PACKAGE_FILE="$ROOT_DIR/forge/apt/extended.txt"
[[ -s $PACKAGE_FILE ]]
...
SIM_FILE=$(mktemp /tmp/hec-apt-extended-sim.XXXXXX)
EST_FILE=$(mktemp /tmp/hec-apt-extended-est.XXXXXX)
...
if grep -qE '^(Remv |The following packages will be REMOVED:)' "$SIM_FILE"; then
  cat "$SIM_FILE" >&2
  echo "extended apt simulation proposed removals" >&2
  exit 1
fi
...
printf 'stage=apt-extended current_available_bytes=%s projected_download_bytes=%s projected_installed_bytes=%s projected_available_bytes=%s reserve_bytes=%s\n' "$CURRENT_BYTES" "$DOWNLOAD_BYTES" "$INSTALLED_BYTES" "$PROJECTED_BYTES" "$RESERVE_BYTES"
if ((PROJECTED_BYTES < RESERVE_BYTES)); then
  echo "extended apt layer cannot fit without violating the disk reserve; no installation performed" >&2
  exit 1
fi
apt-get install -y --no-install-recommends "${PACKAGES[@]}"
for command in buildah java mvn ansible shellcheck shfmt ctags sqlite3 psql mariadb redis-cli mlr vd gnuplot dot pandoc qpdf tesseract convert ffmpeg sox cwebp plantuml ab wrk mkcert mitmproxy; do
  command -v "$command" >/dev/null
 done
printf 'apt-extended packages=%s available_bytes=%s\n' "${#PACKAGES[@]}" "$(df -B1 --output=avail / | tail -n1 | tr -d ' ')"
